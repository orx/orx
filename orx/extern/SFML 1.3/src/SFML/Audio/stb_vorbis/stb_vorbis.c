// Ogg Vorbis I audio decoder  -- version 0.9996
//
...
static int start_decoder(vorb *f)
{
   ...
   for (i=0; i < f->codebook_count; ++i) {
      ...
      if (1 && c->sparse && total >= c->entries >> 2) {
...
static unsigned int vorbis_find_page(stb_vorbis *f, unsigned int *end, int *last)
{
   for(;;) {
      int n;
      if (f->eof) return 0;
      ...
      if (n == 0x4f) { // page header
         ...
         if (i == 4) {
            ...
            if (crc == goal) {
               ...
               // a file with garbage between each ogg page? So
               // even though that paranoia might decrease the
               // chance of an invalid decode by another 2^32,
               // not worth it?
               if (end)
...
unsigned int stb_vorbis_stream_length_in_samples(stb_vorbis *f)
{
   ...
   if (!f->total_samples) {
      ...
      if (f->stream_len >= 65536 && f->stream_len-65536 >= f->first_audio_page_offset)
         set_file_offset(f, f->stream_len - 65536);
      else
...
int stb_vorbis_get_file_offset(stb_vorbis *f)
{
   #ifndef STB_VORBIS_NO_PUSHDATA_API
   if (f->push_mode) return 0;
   #endif
...
static int8 channel_position[7][6] =
{
   ...
   #define FAST_SCALED_FLOAT_TO_INT(x,s) ((temp = (x) + MAGIC(s)), ((int *)&temp)[STB_VORBIS_ENDIAN])
   #define check_endianness()  assert(*(char *) &endian_test == STB_VORBIS_ENDIAN)
   static uint32 endian_test = 0x01FFFF00;
   typedef char stb_vorbis_double_size_test[sizeof(double)==8 && sizeof(int) == 4];
   #define FASTDEF(x) x
#else
...
static void convert_samples_short(int buf_c, short **buffer, int b_offset, int data_c, float **data, int d_offset, int samples)
{
   ...
   if (buf_c != data_c && buf_c <= 2 && data_c <= 6) {
      ...
      for (i=0; i < limit; ++i)
         copy_samples(buffer[i], data[i], samples);
      for (   ; i < buf_c; ++i)
         memset(buffer+i, 0, sizeof(short) * samples);
   }
}
