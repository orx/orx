...
static orxSTATUS orxFASTCALL orxDisplay_Android_CleanGlyph(void *_pContext)
{
  orxSTATUS eResult = orxSTATUS_SUCCESS;
...
static orxSTATUS orxFASTCALL orxDisplay_Android_GenerateGlyph(void *_pContext)
{
     ...
     || (pu8Buffer != orxNULL)))
    {
          ...
          i++)
      {
        ...
        if(pstLoadInfo->bSDF != orxFALSE)
        {
          ...
          orxThread_RunTask(&orxDisplay_Android_GenerateGlyph, &orxDisplay_Android_CleanGlyph, orxNULL, pstGlyphInfo);
        }
        else
        {
          /* Renders the glyph */
