...
typedef struct __orxDISPLAY_WEBP_LOAD_INFO_t
{
  orxU8      *pu8ImageBuffer;
  orxU8      *pu8ImageSource;
  orxS64      s64Size;
...
static const orxSTRING szWebPExtention = ".webp";
...
static void orxDisplay_GLFW_SkipSTBICallback(void *_hResource, unsigned int _uiOffset)
{
  /* Seeks offset */
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_DecompressWebPBitmapCallback(void *_pContext)
{
  orxDISPLAY_EVENT_PAYLOAD  stPayload;
  orxDISPLAY_WEBP_LOAD_INFO     *pstInfo;
  orxU32                    i;
  orxSTATUS                 eResult = orxSTATUS_SUCCESS;

  /* Gets load info */
  pstInfo = (orxDISPLAY_WEBP_LOAD_INFO *)_pContext;
  ...
  /* Frees source */
  WebPFreeDecBuffer(&pstInfo->config.output);
  pstInfo->pu8ImageSource = orxNULL;
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_DecompressWebPBitmap(void *_pContext)
{
  orxDISPLAY_WEBP_LOAD_INFO *pstInfo;
  GLuint                uiBytesPerPixel;
  orxSTATUS             eResult;
  ...
  /* Profiles */
  orxPROFILER_PUSH_MARKER("orxDisplay_DecompressWebPBitmap");

  /* Gets load info */
  pstInfo = (orxDISPLAY_WEBP_LOAD_INFO *)_pContext;

  /* Wants RGBA output */
  pstInfo->config.output.colorspace = MODE_RGBA;

  /* Loads image */
  if(WebPDecode(pstInfo->pu8ImageSource, pstInfo->s64Size, &pstInfo->config) == VP8_STATUS_OK)
  {
    pstInfo->uiWidth = pstInfo->config.output.width;
    pstInfo->uiHeight = pstInfo->config.output.height;

    /* Has NPOT texture support? */
    if(orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NPOT))
    {
      /* Uses image buffer */
      pstInfo->pu8ImageBuffer = (orxU8 *) pstInfo->config.output.u.RGBA.rgba;
    ...
    else
    {
      ...
      {
        /* Copies data */
        orxMemory_Copy(pstInfo->pu8ImageBuffer + uiDstOffset, pstInfo->config.output.u.RGBA.rgba + uiSrcOffset, uiLineSize);
    ...
    /* Stores uncompressed data as new source */
    pstInfo->pu8ImageSource = (orxU8 *) pstInfo->config.output.u.RGBA.rgba;
...
static void orxFASTCALL orxDisplay_GLFW_ReadWebPResourceCallback(orxHANDLE _hResource, orxS64 _s64Size, void *_pBuffer, void *_pContext)
{
  orxDISPLAY_WEBP_LOAD_INFO *pstInfo;
  ...
  if (WebPInitDecoderConfig(&pstInfo->config))
  {
    /* Asynchronous? */
    if(orxFLAG_TEST(pstInfo->pstBitmap->u32Flags, orxDISPLAY_KU32_BITMAP_FLAG_LOADING))
    {
      /* Runs asynchronous task */
      if(orxThread_RunTask(&orxDisplay_GLFW_DecompressWebPBitmap, orxDisplay_GLFW_DecompressWebPBitmapCallback, orxNULL, (void *)pstInfo) == orxSTATUS_FAILURE)
      {
        /* Frees load info */
        orxMemory_Free(pstInfo);
      }
    ...
    {
      /* Decompresses bitmap */
      if(orxDisplay_GLFW_DecompressWebPBitmap(pstInfo) != orxSTATUS_FAILURE)
      {
        /* Upload texture */
        orxDisplay_GLFW_DecompressWebPBitmapCallback(pstInfo);
      }
    }  
  }
  else
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_GetWebPInfos(orxHANDLE _hResource, int *_iWidth, int *_iHeight)
{
  orxSTATUS eResult = orxSTATUS_FAILURE;
  WebPBitstreamFeatures features;
  uint8_t data[64];

  if(orxResource_Read(_hResource, 64, data, orxNULL, orxNULL) == 64)
  {
    VP8StatusCode status = WebPGetFeatures(data, 64, &features);

    if(status == VP8_STATUS_OK)
    {
      *_iWidth = features.width;
      *_iHeight = features.height;
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_LoadWebPBitmapData(orxBITMAP *_pstBitmap)
{
  orxHANDLE hResource;
  orxSTATUS eResult = orxSTATUS_FAILURE;
  ...
  if(hResource != orxHANDLE_UNDEFINED)
  {
    ...
    /* Allocates load info */
    pstInfo = (orxDISPLAY_WEBP_LOAD_INFO *)orxMemory_Allocate(sizeof(orxDISPLAY_WEBP_LOAD_INFO), orxMEMORY_TYPE_TEMP);

    /* Checks */
    orxASSERT(pstInfo != orxNULL);

    /* Inits it */
    orxMemory_Zero(pstInfo, sizeof(orxDISPLAY_WEBP_LOAD_INFO));
    pstInfo->pstBitmap      = _pstBitmap;
    ...
    if(pu8Buffer != orxNULL)
    {
      ...
      if(sstDisplay.pstTempBitmap != orxNULL)
      {
        ...
        /* Gets its info */
        if(orxDisplay_GLFW_GetWebPInfos(hResource, &iWidth, &iHeight) != orxSTATUS_FAILURE)
        {
          /* Resets resource cursor */
          orxResource_Seek(hResource, 0, orxSEEK_OFFSET_WHENCE_START);
          ...
          /* Loads data from resource */
          s64Size = orxResource_Read(hResource, s64Size, pu8Buffer, orxDisplay_GLFW_ReadWebPResourceCallback, (void *)pstInfo);
      ...
      else
      {
        ...
        {
          /* Processes data */
          orxDisplay_GLFW_ReadWebPResourceCallback(hResource, s64Size, (void *)pu8Buffer, (void *)pstInfo);
...
orxBITMAP *orxFASTCALL orxDisplay_GLFW_LoadBitmap(const orxSTRING _zFilename)
{
  ...
  if(zResourceLocation != orxNULL)
  {
    ...
    if(pstResult != orxNULL)
    {
      ...
      /* Loads its data */
      if(orxString_SearchString(_zFilename, szWebPExtention) != orxNULL)
      {
        if(orxDisplay_GLFW_LoadWebPBitmapData(pstResult) == orxSTATUS_FAILURE)
        {
          /* Deletes it */
      ...
      else if(orxDisplay_GLFW_LoadBitmapData(pstResult) == orxSTATUS_FAILURE)
      {
        /* Deletes it */
        orxBank_Free(sstDisplay.pstBitmapBank, pstResult);
