...
static orxINLINE void orxDisplay_SDL_InitShader(const orxDISPLAY_SHADER *_pstShader)
{
  ...
  for(i = 0; i < _pstShader->iTextureCounter; i++)
  {
    ...
    {
      //! TODO: Fix the rotation of the texture
...
orxSTATUS orxFASTCALL orxDisplay_SDL_SetBitmapData(orxBITMAP *_pstBitmap, const orxU8 *_au8Data, orxU32 _u32ByteNumber)
{
  ...
  if((_pstBitmap != sstDisplay.pstScreen) && (_u32ByteNumber == u32Width * u32Height * 4))
  {
    ...
    /* Allocates buffer */
    au8Buffer = (orxU8 *)orxMemory_Allocate(_pstBitmap->u32RealWidth * _pstBitmap->u32RealHeight * 4 * sizeof(orxU8), orxMEMORY_TYPE_VIDEO);

    /* Checks */
    orxASSERT(au8Buffer != orxNULL);

    /* For all visible rows */
    for(i = 0, u32Offset = (_pstBitmap->u32RealWidth - u32Width) * 4, pu8Src = _au8Data, pu8Dst = au8Buffer + ((_pstBitmap->u32RealHeight - u32Height) * _pstBitmap->u32RealWidth * 4);
        i < u32Height;
        i++)
    {
      orxU32 j;
...
orxSTATUS orxFASTCALL orxDisplay_SDL_SaveBitmap(const orxBITMAP *_pstBitmap, const orxSTRING _zFilename)
{
  ...
  if(_pstBitmap == sstDisplay.pstScreen)
  {
    ...
    /* Gets buffer size */
    u32BufferSize = orxF2U(sstDisplay.pstScreen->fWidth * sstDisplay.pstScreen->fHeight) * 4 * sizeof(GLubyte);
