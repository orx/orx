...
#define QOA_NO_STDIO
#define QOA_MALLOC(SIZE)                  orxMemory_Allocate((orxU32)SIZE, orxMEMORY_TYPE_AUDIO)
#define QOA_FREE(MEMORY)                  orxMemory_Free(MEMORY)
...
/** Internal QOA data source
 */
typedef struct __orxSOUNDSYSTEM_QOA_t
{
  ma_data_source_base             stBase;
  orxU64                          u64Cursor;
  ma_read_proc                    pfnRead;
  ma_seek_proc                    pfnSeek;
  ma_tell_proc                    pfnTell;
  void                           *pUserData;
  orxU32                          u32HeaderSize;
  qoa_desc                        stDesc;
  orxS16                          as16Data[0];
...
typedef struct __orxSOUNDSYSTEM_STATIC_t
{
  ...
  ma_data_source_vtable           stQOADataSourceVTable;  /**< QOA data source VTable */
  ma_decoding_backend_vtable      stQOAVTable;            /**< QOA decoding backend VTable */
  ma_decoding_backend_vtable      stVorbisVTable;         /**< Vorbis decoding backend VTable */
  ma_decoding_backend_vtable     *apstVTable[2];          /**< Decoding backend VTable */
        ...
        if(orxMemory_Compare(&(pstFilter->stData), &(stPayload.stFilter.stData), sizeof(orxSOUND_FILTER_DATA)) != 0)
        {
          ...
          switch(pstFilter->stData.eType)
          {
            ...
            case orxSOUND_FILTER_TYPE_BIQUAD:
            {
              ...
              /* Inits its config */
              stConfig = ma_biquad_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), stPayload.stFilter.stData.stBiquad.fB0, stPayload.stFilter.stData.stBiquad.fB1, stPayload.stFilter.stData.stBiquad.fB2, stPayload.stFilter.stData.stBiquad.fA0, stPayload.stFilter.stData.stBiquad.fA1, stPayload.stFilter.stData.stBiquad.fA2);
            ...
            case orxSOUND_FILTER_TYPE_LOW_PASS:
            {
              ...
              else
              {
                ...
                /* Inits its config */
                stConfig = ma_lpf_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stLowPass.fFrequency, stPayload.stFilter.stData.stLowPass.u32Order);
            ...
            case orxSOUND_FILTER_TYPE_HIGH_PASS:
            {
              ...
              else
              {
                ...
                /* Inits its config */
                stConfig = ma_hpf_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stHighPass.fFrequency, stPayload.stFilter.stData.stHighPass.u32Order);
            ...
            case orxSOUND_FILTER_TYPE_BAND_PASS:
            {
              ...
              else
              {
                ...
                /* Inits its config */
                stConfig = ma_bpf_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stBandPass.fFrequency, stPayload.stFilter.stData.stBandPass.u32Order);
            ...
            case orxSOUND_FILTER_TYPE_LOW_SHELF:
            {
              ...
              /* Inits its config */
              stConfig = ma_loshelf2_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stLowShelf.fGain, stPayload.stFilter.stData.stLowShelf.fQ, stPayload.stFilter.stData.stLowShelf.fFrequency);
            ...
            case orxSOUND_FILTER_TYPE_HIGH_SHELF:
            {
              ...
              /* Inits its config */
              stConfig = ma_hishelf2_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stHighShelf.fGain, stPayload.stFilter.stData.stHighShelf.fQ, stPayload.stFilter.stData.stHighShelf.fFrequency);
            ...
            case orxSOUND_FILTER_TYPE_NOTCH:
            {
              ...
              /* Inits its config */
              stConfig = ma_notch2_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stNotch.fQ, stPayload.stFilter.stData.stNotch.fFrequency);
            ...
            case orxSOUND_FILTER_TYPE_PEAKING:
            {
              ...
              /* Inits its config */
              stConfig = ma_peak2_config_init(orxSOUNDSYSTEM_KE_DEFAULT_FORMAT, ma_engine_get_channels(&(sstSoundSystem.stEngine)), pstEngineNode->sampleRate, stPayload.stFilter.stData.stPeaking.fGain, stPayload.stFilter.stData.stPeaking.fQ, stPayload.stFilter.stData.stPeaking.fFrequency);
...
static ma_result orxSoundSystem_MiniAudio_Stream_Read(ma_data_source *_pstDataSource, void *_pFramesOut, ma_uint64 _u64FrameCount, ma_uint64 *_pu64FramesRead)
{
  ...
  *_pu64FramesRead = 0;
  ...
  if(pstSound->bStopping == orxFALSE)
  {
    ...
    {
      /* Has data source? */
      if(pstSound->stStream.eType != orxSOUNDSYSTEM_STREAM_TYPE_EMPTY)
      {
...
static ma_result orxSoundSystem_MiniAudio_QOA_Read(ma_data_source *_pstDataSource, void *_pFramesOut, ma_uint64 _u64FrameCount, ma_uint64 *_pu64FramesRead)
{
  ma_result hResult;

  /* Valid? */
  if((_pstDataSource != NULL) && (_u64FrameCount != 0))
  {
    ...
    /* Has pending data? */
    if(u32FrameRemainder != 0)
    {
      /* Gets frame read count */
      u64FrameRead = (orxU64)orxMIN(QOA_FRAME_LEN - u32FrameRemainder, (orxU32)_u64FrameCount);
      u64FrameRead = orxMIN(u64FrameRead, (orxU64)pstQOA->stDesc.samples - pstQOA->u64Cursor);

      /* Copies data */
      orxMemory_Copy(_pFramesOut, pstQOA->as16Data + u32FrameRemainder * pstQOA->stDesc.channels, (orxU32)u64FrameRead * pstQOA->stDesc.channels * sizeof(orxS16));

      /* Updates cursor */
      pstQOA->u64Cursor += u64FrameRead;
    }
    ...
    /* For all frames */
    while((u64FrameRead < _u64FrameCount) && (pstQOA->u64Cursor < pstQOA->stDesc.samples))
    {
      size_t sSizeRead;
      ...
      /* Updates frames read */
      u64FrameRead += u64FrameCopied;

      /* Updates cursor */
      pstQOA->u64Cursor += u64FrameCopied;
    }

    /* Updates read frames count */
    if(_pu64FramesRead != NULL)
    {
      *_pu64FramesRead = (orxU64)u64FrameRead;
    }

    /* Checks */
    orxASSERT(pstQOA->u64Cursor <= pstQOA->stDesc.samples);

    /* Updates result */
    hResult = (pstQOA->u64Cursor == pstQOA->stDesc.samples) ? MA_AT_END : MA_SUCCESS;
  }
  else
  {
    /* Updates result */
    hResult = MA_INVALID_ARGS;
  }
...
static ma_result orxSoundSystem_MiniAudio_QOA_Seek(ma_data_source *_pstDataSource, ma_uint64 _u64FrameIndex)
{
  ma_result hResult;

  /* Valid? */
  if(_pstDataSource != NULL)
  {
    orxSOUNDSYSTEM_QOA *pstQOA;
    ...
    /* Should read a frame (not at a frame boundary)? */
    if((pstQOA->u64Cursor < pstQOA->stDesc.samples) && (pstQOA->u64Cursor % QOA_FRAME_LEN != 0))
    {
      orxU8 *pu8FrameData;
...
static ma_result orxSoundSystem_MiniAudio_QOA_GetDataFormat(ma_data_source *_pstDataSource, ma_format *_peFormat, ma_uint32 *_pu32Channels, ma_uint32* _pu32SampleRate, ma_channel *_peChannelMap, size_t _sChannelMapCap)
{
  ma_result hResult;
  ...
  if(_pstDataSource != NULL)
  {
    ...
    if(_peFormat != NULL)
    {
      *_peFormat = ma_format_s16;
    }

    /* Updates channels */
    if(_pu32Channels != NULL)
    {
      *_pu32Channels = pstQOA->stDesc.channels;
    }

    /* Updates sample rate */
    if(_pu32SampleRate != NULL)
    {
      *_pu32SampleRate = pstQOA->stDesc.samplerate;
    }

    /* Updates channel map */
    if(_peChannelMap != NULL)
    {
      ma_channel_map_init_standard(ma_standard_channel_map_default, _peChannelMap, _sChannelMapCap, pstQOA->stDesc.channels);
    }
...
static ma_result orxSoundSystem_MiniAudio_QOA_GetCursor(ma_data_source *_pstDataSource, ma_uint64 *_pu64Cursor)
{
  ma_result hResult;

  /* Valid? */
  if((_pstDataSource != NULL) && (_pu64Cursor != NULL))
  {
    orxSOUNDSYSTEM_QOA *pstQOA;
...
static ma_result orxSoundSystem_MiniAudio_QOA_GetLength(ma_data_source *_pstDataSource, ma_uint64 *_pu64Length)
{
  ma_result hResult;
  ...
  if(_pstDataSource != NULL)
  {
    ...
    /* Updates result */
    *_pu64Length = pstQOA->stDesc.samples;
...
static ma_result orxSoundSystem_MiniAudio_InitQOABackend(void *_pUserData, ma_read_proc _pfnRead, ma_seek_proc _pfnSeek, ma_tell_proc _pfnTell, void *_pReadSeekTellUserData, const ma_decoding_backend_config *_pstConfig, const ma_allocation_callbacks *_pstAllocationCallbacks, ma_data_source **_ppstBackend)
{
  ma_result hResult;
  ...
  if(((ma_decoder *)_pReadSeekTellUserData)->onRead == &ma_decoder__on_read_vfs)
  {
    ...
    size_t    sSize;
    orxU32    u32HeaderSize;
    orxU8     au8Buffer[QOA_MIN_FILESIZE];

    /* Is a valid QOA? */
    if((_pfnRead(_pReadSeekTellUserData, au8Buffer, QOA_MIN_FILESIZE, &sSize) == MA_SUCCESS)
    && (sSize == QOA_MIN_FILESIZE)
    && (u32HeaderSize = qoa_decode_header(au8Buffer, QOA_MIN_FILESIZE, &stDesc), u32HeaderSize != 0))
    {
      orxSOUNDSYSTEM_QOA *pstQOA;

      /* Allocates the decoder backend */
      pstQOA = (orxSOUNDSYSTEM_QOA *)ma_malloc(sizeof(orxSOUNDSYSTEM_QOA) + QOA_FRAME_LEN * stDesc.channels * sizeof(orxS16), _pstAllocationCallbacks);

      /* Success? */
      if(pstQOA != NULL)
      {
        ma_data_source_config stDataSourceConfig;

        /* Inits its base */
        stDataSourceConfig        = ma_data_source_config_init();
        stDataSourceConfig.vtable = &(sstSoundSystem.stQOADataSourceVTable);
        hResult                   = ma_data_source_init(&stDataSourceConfig, &(pstQOA->stBase));
        orxASSERT(hResult == MA_SUCCESS);

        /* Inits its descriptor */
        orxMemory_Copy(&(pstQOA->stDesc), &stDesc, sizeof(qoa_desc));

        /* Inits its header size */
        pstQOA->u32HeaderSize = u32HeaderSize;

        /* Inits its cursor */
        pstQOA->u64Cursor = 0;
        _pfnSeek(_pReadSeekTellUserData, pstQOA->u32HeaderSize, ma_seek_origin_start);

        /* Inits its callbacks */
        pstQOA->pfnRead   = _pfnRead;
        pstQOA->pfnSeek   = _pfnSeek;
        pstQOA->pfnTell   = _pfnTell;
        pstQOA->pUserData = _pReadSeekTellUserData;
...
static void orxSoundSystem_MiniAudio_UninitQOABackend(void *_pUserData, ma_data_source *_pstBackend, const ma_allocation_callbacks *_pstAllocationCallbacks)
{
  orxSOUNDSYSTEM_QOA *pstQOA;

  /* Retrieves QOA data source */
  pstQOA = (orxSOUNDSYSTEM_QOA *)_pstBackend;

  /* Uninits its data source */
  ma_data_source_uninit(&(pstQOA->stBase));
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_DeleteSampleTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_DeleteSampleTask, &orxSoundSystem_MiniAudio_FreeSample, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_DeleteTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_DeleteTask, &orxSoundSystem_MiniAudio_FreeSound, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_PlayTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_PlayTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_PauseTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_PauseTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_StopTask(void *_pContext)
{
  ...
  {
    /* Stops sound */
    if(ma_sound_stop(&(pstSound->stSound)) == MA_SUCCESS)
    {
      /* Rewinds it */
      eResult = (ma_sound_seek_to_pcm_frame(&(pstSound->stSound), 0) == MA_SUCCESS) ? orxSTATUS_SUCCESS : orxSTATUS_FAILURE;
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_StopTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_AddFilterTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_AddFilterTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_RemoveLastFilterTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_RemoveLastFilterTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_RemoveAllFiltersTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_RemoveAllFiltersTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetBusTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetBusTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetVolumeTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetVolumeTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetPitchTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetPitchTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetTimeTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetTimeTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetPositionTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetPositionTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetSpatializationTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetSpatializationTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetPanningTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_SetPanningTask, orxNULL, orxNULL, _pContext);
  }
...
static orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_LoopTask(void *_pContext)
{
  ...
  {
    /* Postpones it */
    orxThread_RunTask(&orxSoundSystem_MiniAudio_LoopTask, orxNULL, orxNULL, _pContext);
  }
...
orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_Init()
{
  ...
  if(!(sstSoundSystem.u32Flags & orxSOUNDSYSTEM_KU32_STATIC_FLAG_READY))
  {
    ...
    /* Inits custom decoding backend VTables */
    ...
    sstSoundSystem.stQOAVTable.onInit                                     = &orxSoundSystem_MiniAudio_InitQOABackend;
    sstSoundSystem.stQOAVTable.onUninit                                   = &orxSoundSystem_MiniAudio_UninitQOABackend;
    sstSoundSystem.apstVTable[1]                                          = &(sstSoundSystem.stQOAVTable);

    /* Inits data source VTables */
    ...
    sstSoundSystem.stQOADataSourceVTable.onRead                           = &orxSoundSystem_MiniAudio_QOA_Read;
    sstSoundSystem.stQOADataSourceVTable.onSeek                           = &orxSoundSystem_MiniAudio_QOA_Seek;
    sstSoundSystem.stQOADataSourceVTable.onGetDataFormat                  = &orxSoundSystem_MiniAudio_QOA_GetDataFormat;
    sstSoundSystem.stQOADataSourceVTable.onGetCursor                      = &orxSoundSystem_MiniAudio_QOA_GetCursor;
    sstSoundSystem.stQOADataSourceVTable.onGetLength                      = &orxSoundSystem_MiniAudio_QOA_GetLength;
...
orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_DeleteSample(orxSOUNDSYSTEM_SAMPLE *_pstSample)
{
  ...
  eResult = orxThread_RunTask(&orxSoundSystem_MiniAudio_DeleteSampleTask, &orxSoundSystem_MiniAudio_FreeSample, orxNULL, _pstSample);
...
orxSOUNDSYSTEM_SOUND *orxFASTCALL orxSoundSystem_MiniAudio_CreateStream(orxHANDLE _hUserData, orxSOUNDSYSTEM_STREAM_TYPE _eType, orxU32 _u32ChannelNumber, orxU32 _u32SampleRate)
{
  ...
  /* Valid parameters? */
  if((_eType < orxSOUNDSYSTEM_STREAM_TYPE_NUMBER) && (_u32ChannelNumber >= 1) && (_u32ChannelNumber <= 2) && (_u32SampleRate > 0))
  {
    /* Allocates sound */
    pstResult = (orxSOUNDSYSTEM_SOUND *)orxBank_Allocate(sstSoundSystem.pstSoundBank);
    ...
    if(pstResult != orxNULL)
    {
      ...
      {
        /* Logs message */
        orxDEBUG_PRINT(orxDEBUG_LEVEL_SOUND, "Can't create noise sound stream with [%u] channels: can't initialize noise.", _u32ChannelNumber);
...
orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_Delete(orxSOUNDSYSTEM_SOUND *_pstSound)
{
  ...
  /* Runs delete task */
  eResult = orxThread_RunTask(&orxSoundSystem_MiniAudio_DeleteTask, &orxSoundSystem_MiniAudio_FreeSound, orxNULL, _pstSound);
