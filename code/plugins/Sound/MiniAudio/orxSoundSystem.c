...
static ma_result SoundSystem_MiniAudio_Stream_GetDataFormat(ma_data_source *_pstDataSource, ma_format *_peFormat, ma_uint32 *_pu32Channels, ma_uint32* _pu32SampleRate, ma_channel *_peChannelMap, size_t _sChannelMapCap)
{
  ...
  {
    *_pu32Channels = pstSound->stStream.u32ChannelNumber;
  }
  if(_pu32SampleRate != NULL)
  {
    *_pu32SampleRate = pstSound->stStream.u32SampleRate;
  }
...
orxSOUNDSYSTEM_SAMPLE *orxFASTCALL orxSoundSystem_MiniAudio_LoadSample(const orxSTRING _zFilename)
{
  ...
  if(pstResult != orxNULL)
  {
    ...
    if(zResourceLocation != orxNULL)
    {
      ...
      /* Inits data source */
      hResult = ma_resource_manager_data_source_init(&(sstSoundSystem.stResourceManager), zResourceLocation, MA_RESOURCE_MANAGER_DATA_SOURCE_FLAG_WAIT_INIT | MA_RESOURCE_MANAGER_DATA_SOURCE_FLAG_DECODE | MA_RESOURCE_MANAGER_DATA_SOURCE_FLAG_ASYNC, 0, NULL, &(pstResult->stResource.stDataSource));
  ...
  if((_u32ChannelNumber >= 1) && (_u32ChannelNumber <= 2) && (_u32SampleRate > 0))
  {
    ...
    if(pstResult != orxNULL)
    {
      ...
      if(hResult == MA_SUCCESS)
      {
        ...
        /* Stores info */
        pstResult->stStream.u32ChannelNumber = _u32ChannelNumber;
        pstResult->stStream.u32SampleRate    = _u32SampleRate;
...
orxSOUNDSYSTEM_SOUND *orxFASTCALL orxSoundSystem_MiniAudio_CreateStreamFromFile(orxHANDLE _hUserData, const orxSTRING _zFilename)
{
  ...
  if(zResourceLocation != orxNULL)
  {
    ...
    if(pstResult != orxNULL)
    {
      ...
      /* Inits data source */
      hResult = ma_resource_manager_data_source_init(&(sstSoundSystem.stResourceManager), zResourceLocation, MA_RESOURCE_MANAGER_DATA_SOURCE_FLAG_WAIT_INIT | MA_RESOURCE_MANAGER_DATA_SOURCE_FLAG_STREAM, 0, NULL, &(pstResult->stStream.stDataSource));
...
orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_SetSpatialization(orxSOUNDSYSTEM_SOUND *_pstSound, orxFLOAT _fInnerDistance, orxFLOAT _fOuterDistance, orxFLOAT _fInnerVolume, orxFLOAT _fOuterVolume, orxFLOAT _fRollOff)
{
  orxSTATUS eResult;
  ...
  {
    /* Updates spatialization status */
    ma_sound_set_spatialization_enabled(&(_pstSound->stSound), ((_fInnerDistance >= orxFLOAT_0) && (_fOuterDistance >= orxFLOAT_0)) ? orxTRUE : orxFALSE);

    /* Updates spatialization properties */
    ma_sound_set_min_distance(&(_pstSound->stSound), _fInnerDistance);
    ma_sound_set_max_distance(&(_pstSound->stSound), _fOuterDistance);
    ma_sound_set_min_gain(&(_pstSound->stSound), _fOuterVolume);
    ma_sound_set_max_gain(&(_pstSound->stSound), _fInnerVolume);
...
orxSTATUS orxFASTCALL orxSoundSystem_MiniAudio_GetSpatialization(const orxSOUNDSYSTEM_SOUND *_pstSound, orxFLOAT *_pfInnerDistance, orxFLOAT *_pfOuterDistance, orxFLOAT *_pfInnerVolume, orxFLOAT *_pfOuterVolume, orxFLOAT *_pfRollOff)
{
  orxSTATUS eResult;
  ...
  if(sstSoundSystem.u32ListenerNumber > 0)
  {
    ...
    {
      /* Updates values */
      *_pfInnerDistance = ma_sound_get_min_distance(&(_pstSound->stSound));
      *_pfOuterDistance = ma_sound_get_max_distance(&(_pstSound->stSound));
      *_pfOuterVolume   = ma_sound_get_min_gain(&(_pstSound->stSound));
      *_pfInnerVolume   = ma_sound_get_max_gain(&(_pstSound->stSound));
      *_pfRollOff       = ma_sound_get_rolloff(&(_pstSound->stSound));
    }
    else
    {
      /* Updates values */
      *_pfInnerDistance = *_pfOuterDistance = -orxFLOAT_1;
      *_pfInnerVolume   = *_pfOuterVolume   = orxFLOAT_1;
