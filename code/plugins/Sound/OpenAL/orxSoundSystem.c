...
static void orxFASTCALL orxSoundSystem_OpenAL_FillStream(orxSOUNDSYSTEM_SOUND *_pstSound)
{
  ...
  if(_pstSound->bStop == orxFALSE)
  {
    ...
    if(iBufferNumber > 0)
    {
      ...
      u32BufferFrameNumber = (orxSOUNDSYSTEM_KU32_STREAM_BUFFER_SIZE / sizeof(orxU16)) / _pstSound->stData.u32ChannelNumber;
      ...
      for(i = 0, u32FrameNumber = u32BufferFrameNumber; i < (orxU32)iBufferNumber; i++)
      {
        ...
        /* Fills buffer */
        u32FrameNumber = orxSoundSystem_OpenAL_Read(&(_pstSound->stData), u32BufferFrameNumber, sstSoundSystem.au8StreamBuffer);

        /* Success? */
        if(u32FrameNumber > 0)
        {
          /* Transfers its data */
          alBufferData(puiBufferList[i], (_pstSound->stData.u32ChannelNumber > 1) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16, sstSoundSystem.au8StreamBuffer, (ALsizei)(u32FrameNumber * _pstSound->stData.u32ChannelNumber * sizeof(orxU16)), (ALsizei)_pstSound->stData.u32SampleRate);
          alASSERT();
