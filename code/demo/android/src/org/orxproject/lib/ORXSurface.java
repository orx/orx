...
public class ORXSurface extends SurfaceView implements SurfaceHolder.Callback,
	...
	public ORXSurface(Context context) {
		...
		Log.i("orx app", "build new surface");
		int width = getWidth();
	...
	public void surfaceCreated(SurfaceHolder holder) {
		Log.v("ORX", "Surface created or resume from pausing");
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
			int height) {
		synchronized (AnOrxActivity.ORXThreadLock) {
			Log.i("ORX", "Surface resized" + width + "," + height);
	...
	public boolean createEGLSurface() {
		...
		mEGLSurface = surface;
		Log.i("orx app", "make " + isMake + "eglsurface create " + mEGLSurface
	...
	private static class ConfigChooser implements
		...
		public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {
			...
			if (numConfigs <= 0) {
				Log.i("ORX", "do not support gles 2.0");
				gles2Support = false;
			...
			Log.i("ORX", "support gles 2.0 shader is coming");
			gles2Support = true;
