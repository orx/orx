...
#define orxINSPECTOR_KZ_HIGHLIGHT           "Highlight"
...
static const orxSTRING orxInspector_PrintGUID(const orxOBJECT *_pstObject, const orxSTRING _zPrefix = orxNULL)
{
...
static orxVECTOR *orxInspector_GetHighlightColor(const orxOBJECT *_pstObject, orxVECTOR *_pvColor)
{
  orxCOLOR  stColor;
  orxU64    u64GUID;
...
static orxSTATUS orxFASTCALL orxInspector_EventHandler(const orxEVENT *_pstEvent)
{
  // Render?
  if(_pstEvent->eType == orxEVENT_TYPE_RENDER)
  {
    // Has highlight shader?
    if(sstInspector.pstShader != orxNULL)
    {
      ...
      if((zKey = orxInspector_PrintGUID(pstObject), (orxConfig_HasValueNoCheck(zKey) != orxFALSE) && (orxConfig_GetBool(zKey) != orxFALSE))
      && (zKey = orxInspector_PrintGUID(pstObject, orxINSPECTOR_KZ_HIGHLIGHT), (orxConfig_HasValueNoCheck(zKey) != orxFALSE) && (orxConfig_GetBool(zKey) != orxFALSE)))
      {
        // Render object start?
        if(_pstEvent->eID == orxRENDER_EVENT_OBJECT_START)
        {
          // Starts highlight shader
          orxShader_Start(sstInspector.pstShader, orxSTRUCTURE(_pstEvent->hSender));
        }
        else
        {
          // Stops highglight shader
          orxShader_Stop(sstInspector.pstShader);
        }
      }
  ...
  else
  {
    ...
    // Inspector highlight?
    if((orxString_Compare(pstPayload->zShaderName, orxShader_GetName(sstInspector.pstShader)) == 0)
...
static void orxInspector_ShowConfigRow(const orxSTRING _zKey, orxBOOL _bForceUpdate = orxFALSE, const orxSTRING _zSection = orxNULL)
{
  ImGui::TableNextRow();
...
static void orxInspector_ShowObjectGUID(const orxOBJECT *_pstObject)
{
  // Get object guid
...
static void orxInspector_ShowObjectHeader()
{
...
static void orxInspector_ShowObjectButton(orxOBJECT *_pstObject)
{
  ImGui::TableNextColumn();
...
static void orxInspector_ShowParent(const orxOBJECT *_pstObject)
{
...
static void orxInspector_ShowSiblings(const orxOBJECT *_pstObject)
{
  orxOBJECT *pstParent = orxOBJECT(orxObject_GetParent(_pstObject));
  if(pstParent != orxNULL)
  {
...
static void orxInspector_ShowChildren(const orxOBJECT *_pstObject)
{
...
static void orxInspector_ShowStatus(const orxOBJECT *_pstObject)
{
...
static void orxInspector_ShowConfig(const orxSTRING _zSection)
{
...
static void orxInspector_ShowHighlightCheckbox(orxOBJECT *_pstObject)
{
  ...
  if(orxObject_GetWorkingGraphic(_pstObject) != orxNULL)
  {
    ...
    ImGui::TextColored(vHighlight, orxINSPECTOR_KZ_HIGHLIGHT);
    ImGui::SameLine();

    orxConfig_PushSection(orxINSPECTOR_KZ_CONFIG_RUNTIME);
    const orxSTRING zHighlight = orxInspector_PrintGUID(_pstObject, orxINSPECTOR_KZ_HIGHLIGHT);
    bool bHighlight = (orxConfig_HasValueNoCheck(zHighlight) != orxFALSE) && (orxConfig_GetBool(zHighlight) != orxFALSE);
    if(ImGui::Checkbox("##highlight", &bHighlight))
    {
      orxConfig_SetBool(zHighlight, bHighlight ? orxTRUE : orxFALSE);
    }
    orxConfig_PopSection();
  }
...
static void orxInspector_ShowShader(const orxOBJECT *_pstObject)
{
  const orxSHADER *pstShader = orxObject_GetShader(_pstObject);
  if(pstShader != orxNULL)
  {
    if(ImGui::CollapsingHeader("Shaders") && ImGui::BeginTable("Shaders", 2, orxINSPECTOR_KU32_TABLE_FLAGS))
    {
        ImGui::TableNextColumn();
...
static void orxInspector_ShowGraphic(const orxOBJECT *_pstObject)
{
...
static void orxInspector_ShowObject(orxOBJECT *_pstObject)
{
  ...
  if(ImGui::Begin(orxInspector_PrintGUID(_pstObject, zName), &bRegistered))
  {
    ...
    orxInspector_ShowShader(_pstObject);
...
static void orxInspector_CommandRegisterObject(orxU32 _u32ArgNumber, const orxCOMMAND_VAR *_astArgList, orxCOMMAND_VAR *_pstResult)
{
  orxOBJECT *pstObject;
...
static void orxInspector_CommandUnregisterObject(orxU32 _u32ArgNumber, const orxCOMMAND_VAR *_astArgList, orxCOMMAND_VAR *_pstResult)
{
  orxOBJECT *pstObject;
...
static void orxInspector_CommandIsObjectRegistered(orxU32 _u32ArgNumber, const orxCOMMAND_VAR *_astArgList, orxCOMMAND_VAR *_pstResult)
{
  orxOBJECT *pstObject;
...
static void orxInspector_Update(const orxCLOCK_INFO *_pstClockInfo, void *_pContext)
{
  // Push input set
...
void orxInspector_Init()
{
  ...
  if(sstInspector.bInit == orxFALSE)
  {
    ...
    if(orxConfig_HasSection(orxINSPECTOR_KZ_CONFIG_SECTION) != orxFALSE)
    {
      ...
      // Create shader
      const orxSTRING zShader = (orxConfig_HasValue(orxINSPECTOR_KZ_CONFIG_SHADER) != orxFALSE) ? orxConfig_GetString(orxINSPECTOR_KZ_CONFIG_SHADER) : orxINSPECTOR_KZ_CONFIG_SECTION;
      sstInspector.pstShader = orxShader_CreateFromConfig(zShader);
      ...
      // Add event handler
      orxEvent_AddHandler(orxEVENT_TYPE_RENDER, orxInspector_EventHandler);
      orxEvent_SetHandlerIDFlags(orxInspector_EventHandler, orxEVENT_TYPE_RENDER, orxNULL, orxEVENT_GET_FLAG(orxRENDER_EVENT_OBJECT_START) | orxEVENT_GET_FLAG(orxRENDER_EVENT_OBJECT_STOP), orxEVENT_KU32_MASK_ID_ALL);
      orxEvent_AddHandler(orxEVENT_TYPE_SHADER, orxInspector_EventHandler);
      orxEvent_SetHandlerIDFlags(orxInspector_EventHandler, orxEVENT_TYPE_SHADER, orxNULL, orxEVENT_GET_FLAG(orxSHADER_EVENT_SET_PARAM), orxEVENT_KU32_MASK_ID_ALL);
...
void orxInspector_Exit()
{
  ...
  if(sstInspector.bInit != orxFALSE)
  {
    ...
    orxEvent_RemoveHandler(orxEVENT_TYPE_RENDER, orxInspector_EventHandler);
...
orxSTATUS orxInspector_RegisterObject(orxOBJECT *_pstObject)
{
  ...
  if(_pstObject != orxNULL)
  {
    ...
    const orxSTRING zHighlight = orxInspector_PrintGUID(_pstObject, orxINSPECTOR_KZ_HIGHLIGHT);
    if(!orxConfig_HasValueNoCheck(zHighlight))
    {
      orxConfig_SetBool(zHighlight, orxTRUE);
    }
    orxConfig_PopSection();
