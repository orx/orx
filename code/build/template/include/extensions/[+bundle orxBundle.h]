...
#define orxBUNDLE_KU32_BATCH_SIZE           (2 * orxTHREAD_KU32_MAX_THREAD_NUMBER)
...
#define orxBUNDLE_KU32_COMPRESSION_LEVEL    LZ4HC_CLEVEL_MAX

#define orxBUNDLE_KZ_BINARY_TAG             "OBR1"
#define orxBUNDLE_KU32_HEADER_INTRO_SIZE    (4 + 4)
#define orxBUNDLE_KU32_HEADER_ENTRY_SIZE    (8 + 8 + 4 + 4)
...
typedef struct BundleResourceRef
{
  orxSTRINGID     stNameID;
...
typedef struct __orxBUNDLE_t
{
  ...
  orxHANDLE       hResource;
  orxHANDLE       hOutput;
  volatile orxU32 u32CurrentResourceID;
  orxU32          u32DataCount;
  orxS32          s32Monitor;
  orxBOOL         bProcess;
  orxBOOL         bInit;
...
static orxSTATUS orxFASTCALL orxBundle_CleanResource(void *_pContext)
{
  BundleResourceRef *pstResourceRef;

  // Gets resource ref
  pstResourceRef = (BundleResourceRef *)_pContext;

  // Is opened?
  if(pstResourceRef->hResource != orxHANDLE_UNDEFINED)
  {
    // Closes it
    orxResource_Close(pstResourceRef->hResource);
...
static orxSTATUS orxFASTCALL orxBundle_ProcessResource(void *_pContext)
{
  BundleResourceRef  *pstResourceRef;
  ...
  // Is size supported?
  if(pstResourceRef->s64FinalSize <= 0x7FFFFFFF)
  {
    orxU8 *pu8Buffer;
    ...
    {
      // Reads data
      if(orxResource_Read(pstResourceRef->hResource, pstResourceRef->s64FinalSize, pu8Buffer, orxNULL, orxNULL) == pstResourceRef->s64FinalSize)
      {
        ...
        // Success?
        if(pu8CompressedBuffer != orxNULL)
        {
          // Compresses it
          s32CompressedSize = (orxS32)LZ4_compress_HC((const char *)pu8Buffer, (char *)pu8CompressedBuffer, (int)pstResourceRef->s64FinalSize, (int)s32CompressedSize, orxBUNDLE_KU32_COMPRESSION_LEVEL);
          ...
          if(s32CompressedSize > 0)
          {
            ...
            // Waits for its turn
            while(sstBundle.u32CurrentResourceID != pstResourceRef->u32ID)
            {
              orxThread_Yield();
            }

            // Binary output?
            if(sstBundle.hOutput == sstBundle.hResource)
            {
              // Outputs resource
              orxResource_Write(sstBundle.hOutput, (orxS64)s32CompressedSize, pu8CompressedBuffer, orxNULL, orxNULL);
            }
            else
            {
              orxS32 s32Index;
              ...
              orxResource_Print(sstBundle.hOutput, "static const orxU8 BundleData0x%x[] =\r\n{", pstResourceRef->u32ID);
              ...
              for(s32Index = 0; s32Index < s32CompressedSize; s32Index++)
              {
                ...
                {
                  // Outputs it
                  orxResource_Print(sstBundle.hOutput, "\r\n ");
                }
                ...
                orxResource_Write(sstBundle.hOutput, sizeof(acByteBuffer), acByteBuffer, orxNULL, orxNULL);
              }

              // Outputs structure footer
              if(s32Index > 0)
              {
                orxResource_Seek(sstBundle.hOutput, -1, orxSEEK_OFFSET_WHENCE_CURRENT);
              }
              orxResource_Print(sstBundle.hOutput, "\r\n};\r\n\r\n");
            }
          ...
          {
            // Logs message
            orxLOG(orxBUNDLE_KZ_LOG_TAG orxANSI_KZ_COLOR_FG_RED "Failed to compress " orxANSI_KZ_COLOR_FG_MAGENTA "[%s]" orxANSI_KZ_COLOR_RESET ", skipping!", orxString_GetFromID(pstResourceRef->stNameID));
          }
        ...
        {
          // Logs message
          orxLOG(orxBUNDLE_KZ_LOG_TAG orxANSI_KZ_COLOR_FG_RED "Failed to allocate %s for " orxANSI_KZ_COLOR_FG_MAGENTA "[%s]" orxANSI_KZ_COLOR_RESET ", skipping!", orxBundle_GetHumanReadableSize(s32CompressedSize, 2), orxString_GetFromID(pstResourceRef->stNameID));
        }
      }
      else
      {
        // Logs message
        orxLOG(orxBUNDLE_KZ_LOG_TAG orxANSI_KZ_COLOR_FG_RED "Failed to read " orxANSI_KZ_COLOR_FG_MAGENTA "[%s]" orxANSI_KZ_COLOR_RESET ", skipping!", orxString_GetFromID(pstResourceRef->stNameID));
      }
    ...
    {
      // Logs message
      orxLOG(orxBUNDLE_KZ_LOG_TAG orxANSI_KZ_COLOR_FG_RED "Failed to allocate %s for " orxANSI_KZ_COLOR_FG_MAGENTA "[%s]" orxANSI_KZ_COLOR_RESET ", skipping!", orxBundle_GetHumanReadableSize(pstResourceRef->s64FinalSize, 2), orxString_GetFromID(pstResourceRef->stNameID));
    }
  }
  else
  {
    // Logs message
    orxLOG(orxBUNDLE_KZ_LOG_TAG orxANSI_KZ_COLOR_FG_RED "Size too large (%s) for " orxANSI_KZ_COLOR_FG_MAGENTA "[%s]" orxANSI_KZ_COLOR_RESET ", skipping!", orxBundle_GetHumanReadableSize(pstResourceRef->s64FinalSize, 2), orxString_GetFromID(pstResourceRef->stNameID));
  }
  ...
  // Updates current resource ID: next!
  orxMEMORY_BARRIER();
  sstBundle.u32CurrentResourceID = sstBundle.u32CurrentResourceID + 1;
...
static orxINLINE orxSTATUS orxBundle_Process()
{
  ...
  sstBundle.hOutput = (sstBundle.hResource != orxHANDLE_UNDEFINED) ? sstBundle.hResource : orxResource_Open(orxResource_LocateInStorage(orxBUNDLE_KZ_RESOURCE_GROUP, orxNULL, orxBUNDLE_KZ_INCLUDE_PATH orxBUNDLE_KZ_INCLUDE_FILENAME), orxTRUE);

  // Success?
  if(sstBundle.hOutput != orxHANDLE_UNDEFINED)
  {
    struct
    {
      const orxSTRING zKey;
    ...
    pstResourceBank = orxBank_Create(orxBUNDLE_KU32_TABLE_SIZE, sizeof(BundleResourceRef), orxBANK_KU32_FLAG_NONE, orxMEMORY_TYPE_TEMP);
    ...
    // Updates status
    bBinary = (sstBundle.hOutput == sstBundle.hResource) ? orxTRUE : orxFALSE;
                ...
                if(hResource != orxHANDLE_UNDEFINED)
                {
                  ...
                  if(bCollect != orxFALSE)
                  {
                    ...
                    if((s64Size = orxResource_GetSize(hResource)) > 0)
                    {
                      ...
                      // Adds ref
                      pstResourceRef = (BundleResourceRef *)orxBank_Allocate(pstResourceBank);
                      ...
                      pstResourceRef->u32ID         = orxU32_UNDEFINED;
                      pstResourceRef->s64FinalSize  = s64Size;
                    }
                    else
                    {
                      // Logs message
                      orxLOG(orxBUNDLE_KZ_LOG_TAG "Skipping " orxBUNDLE_KZ_RESOURCE_FORMAT orxANSI_KZ_COLOR_FG_RED " (empty)", zGroup, zName);
                    }
                  }
                  else
                  {
    ...
    {
      BundleResourceRef  *pstResourceRef, *pstNextResourceRef;
      ...
      if(bBinary != orxFALSE)
      {
        ...
        orxResource_Seek(sstBundle.hOutput, u32HeaderSize, orxSEEK_OFFSET_WHENCE_START);
      }

      // Inits current resource ID
      sstBundle.u32CurrentResourceID = 0;

      // For all refs
      for(pstResourceRef = (BundleResourceRef *)orxBank_GetNext(pstResourceBank, orxNULL), u32ResourceIndex = 0;
          pstResourceRef != orxNULL;
          pstResourceRef = pstNextResourceRef)
      {
        // Gets next resource ref
        pstNextResourceRef = (BundleResourceRef *)orxBank_GetNext(pstResourceBank, pstResourceRef);
        ...
        pstResourceRef->hResource = orxResource_Open(pstResourceRef->zLocation, orxFALSE);

        // Success?
        if(pstResourceRef->hResource != orxHANDLE_UNDEFINED)
        {
          // Updates task
          pstResourceRef->u32ID = u32ResourceIndex++;
          orxMEMORY_BARRIER();

          // Binary mode?
          if(bBinary != orxFALSE)
          {
            // Prevents task overload (OS file descriptors limit)
            while(orxThread_GetTaskCount() > orxBUNDLE_KU32_BATCH_SIZE)
            {
              orxThread_Yield();
            }

            // Runs task in parallel
            orxThread_RunTask(&orxBundle_ProcessResource, &orxBundle_CleanResource, orxNULL, pstResourceRef);
          }
          else
          {
            // Runs task locally (faster in non-binary mode)
            orxBundle_ProcessResource(pstResourceRef);
            orxBundle_CleanResource(pstResourceRef);
          }
        }
        else
        {
          // Logs message
          orxLOG(orxBUNDLE_KZ_LOG_TAG orxANSI_KZ_COLOR_FG_RED "Failed to open " orxANSI_KZ_COLOR_FG_MAGENTA "[%s]" orxANSI_KZ_COLOR_RESET ", skipping!", orxString_GetFromID(pstResourceRef->stNameID));
        }
      }
      ...
      if(orxBank_GetCount(pstResourceBank) != 0)
      {
        ...
        {
          // Inits header
          orxResource_Seek(sstBundle.hOutput, 0, orxSEEK_OFFSET_WHENCE_START);
          orxResource_Print(sstBundle.hOutput, orxBUNDLE_KZ_BINARY_TAG);
          orxResource_WriteU32(sstBundle.hOutput, orxBank_GetCount(pstResourceBank));

          // For all refs
          for(pstResourceRef = (BundleResourceRef *)orxBank_GetNext(pstResourceBank, orxNULL), u32ResourceIndex = 0;
              pstResourceRef != orxNULL;
              pstResourceRef = (BundleResourceRef *)orxBank_GetNext(pstResourceBank, pstResourceRef), u32ResourceIndex++)
          {
            // Valid?
            if(pstResourceRef->s64Size > 0)
            {
              // Writes its header entry
              orxResource_WriteU64(sstBundle.hOutput, (orxU64)pstResourceRef->stNameID);
              orxResource_WriteU64(sstBundle.hOutput, (orxU64)u32HeaderSize + s64Size);
              orxResource_WriteU32(sstBundle.hOutput, (orxU32)pstResourceRef->s64Size);
              orxResource_WriteU32(sstBundle.hOutput, (orxU32)pstResourceRef->s64FinalSize);
            ...
            {
              // Writes empty entry
              orxResource_WriteU64(sstBundle.hOutput, 0);
              orxResource_WriteU64(sstBundle.hOutput, 0);
              orxResource_WriteU32(sstBundle.hOutput, 0);
              orxResource_WriteU32(sstBundle.hOutput, 0);
            }
          }
        }
        else
        {
          ...
          orxResource_Print(sstBundle.hOutput, "static const BundleData sastBundleDataList[] =\r\n{");
        ...
        orxLOG(orxBUNDLE_KZ_LOG_TAG "Bundled " orxANSI_KZ_COLOR_FG_CYAN "%u" orxANSI_KZ_COLOR_RESET " resources into " orxANSI_KZ_COLOR_FG_MAGENTA "%s" orxANSI_KZ_COLOR_RESET " in " orxANSI_KZ_COLOR_FG_CYAN "%.2f" orxANSI_KZ_COLOR_RESET "s, " orxANSI_KZ_COLOR_FG_GREEN "(%s) -> [%s]" orxANSI_KZ_COLOR_FG_CYAN " (%.2f%%)" orxANSI_KZ_COLOR_RESET,
              u32ResourceCount,
              orxResource_GetPath(orxResource_GetLocation(sstBundle.hOutput)),
    ...
    orxResource_Close(sstBundle.hOutput);
    if(sstBundle.hOutput == sstBundle.hResource)
    {
      sstBundle.hResource = orxHANDLE_UNDEFINED;
    }
    sstBundle.hOutput = orxHANDLE_UNDEFINED;
  }
  else
  {
    // Logs message
        ...
        && (orxMemory_Compare(orxBUNDLE_KZ_BINARY_TAG, acTag, 4) == 0))
        {
          ...
          && (u32Index < orxResource_ReadU32(hResource)))
          {
            ...
            if(pstResource != orxNULL)
            {
              ...
              // Skips to its info
              orxResource_Seek(hResource, orxBUNDLE_KU32_HEADER_INTRO_SIZE + u32Index * orxBUNDLE_KU32_HEADER_ENTRY_SIZE + 8 + 8, orxSEEK_OFFSET_WHENCE_START);
...
static orxS64 orxFASTCALL orxBundle_Read(orxHANDLE _hResource, orxS64 _s64Size, void *_pu8Buffer)
{
  ...
  if(pstResource->pu8FinalBuffer == orxNULL)
  {
    ...
    if(pstResource->stData.pu8Buffer == orxNULL)
    {
      ...
      // Skips to content
      orxResource_Seek(hResource, (orxS64)orxResource_ReadU64(hResource), orxSEEK_OFFSET_WHENCE_START);
