...
static void orxFASTCALL orxFont_UpdateMap(orxFONT *_pstFont)
{
  ...
  if(orxStructure_TestFlags(_pstFont, orxFONT_KU32_FLAG_CAN_UPDATE_MAP))
  {
    ...
    && (_pstFont->zCharacterList != orxSTRING_EMPTY))
    {
      ...
      for(s32Index = 0, u32CharacterCodePoint = orxString_GetFirstCharacterCodePoint(_pstFont->zCharacterList, &pc), orxVector_Set(&vOrigin, _pstFont->fLeft + _pstFont->vCharacterSpacing.fX, _pstFont->fTop + _pstFont->vCharacterSpacing.fY, orxFLOAT_0);
          ...
          s32Index++, u32CharacterCodePoint = orxString_GetFirstCharacterCodePoint(pc, &pc))
      {
        ...
        {
          /* Reinits its X value */
          vOrigin.fX = _pstFont->fLeft + _pstFont->vCharacterSpacing.fX;
  ...
  && (orxConfig_HasSection(_pstFont->zReference) != orxFALSE))
  {
    ...
    if(eResult != orxSTATUS_FAILURE)
    {
      ...
      if(bSDF != orxFALSE)
      {
        ...
        {
          orxVECTOR       vSize;
          orxCHAR         acBuffer[256], acShaderBuffer[256];
