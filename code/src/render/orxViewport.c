  ...
  && (orxConfig_PushSection(_zConfigID) != orxSTATUS_FAILURE))
  {
    ...
    if(pstResult != orxNULL)
    {
      ...
      if(orxConfig_HasValue(orxVIEWPORT_KZ_CONFIG_BACKGROUND_COLOR) != orxFALSE)
      {
        ...
        /* Gets color vector */
        orxConfig_GetVector(orxVIEWPORT_KZ_CONFIG_BACKGROUND_COLOR, &(stColor.vRGBA));
        orxVector_Mulf(&(stColor.vRGBA), &(stColor.vRGBA), orxCOLOR_NORMALIZER);
        stColor.vRGBA.fA = (orxConfig_HasValue(orxVIEWPORT_KZ_CONFIG_BACKGROUND_ALPHA) != orxFALSE) ? orxConfig_GetFloat(orxVIEWPORT_KZ_CONFIG_BACKGROUND_ALPHA) : orxFLOAT_1;
...
orxSTATUS orxFASTCALL orxViewport_ClearBackgroundColor(orxVIEWPORT *_pstViewport)
{
  ...
  orxVector_Copy4(&(_pstViewport->stBackgroundColor.vRGBA), &orxVECTOR_BLACK);
