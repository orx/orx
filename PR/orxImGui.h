...
typedef struct __orxIMGUI_t
{
  ...
  const ImFontConfig                       *pstDefaultImFontConfig;
...
//   sstImGui.pstDefaultFont = rstIO.Fonts->AddFontDefault();
  sstImGui.pstDefaultFont = rstIO.Fonts->AddFontDefault(sstImGui.pstDefaultImFontConfig);
        ...
        if(hFont != orxHANDLE_UNDEFINED)
        {
          ...
          if(pBuffer)
          {
            if(orxResource_Read(hFont, s32Size, pBuffer, orxNULL, orxNULL) == s32Size)
            {
              ...
              strncpy(font_cfg.Name, zFont, sizeof(font_cfg.Name) - 1); 
              rstIO.Fonts->AddFontFromMemoryTTF(pBuffer, s32Size, fFontSize, &font_cfg, NULL);
